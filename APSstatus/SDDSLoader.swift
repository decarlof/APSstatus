    ...
    func fetchStatus() {
        ...
        Task {
            do {
                ...
                let (parsedItems, status) = try await Task.detached(priority: .userInitiated) { () -> ([(String,String)], String) in
                    let decompressed = try compressedData.gunzipped()
                    let parsed = try parseMainStatusItems(decompressed)
                    return (parsed, "Loaded SDDS (\(parsed.count) items)")
                }.value
...
// MARK: - Pure parsing helper for mainStatus (runs off-main)
func parseMainStatusItems(_ data: Data) throws -> [(description: String, value: String)] {
    // This is your existing parseSDDS logic, but:
    // - Do not touch any @Published here
    // - At the end, return [(description, value)]

    // ... copy the same parsing code you already have ...
    // Replace the final assignment with `return results`

    // The following block is the same as your current parseSDDS,
    // adapted to return `results` instead of `extractedData = results`.
    ...
    guard !columns.isEmpty else { throw NSError(domain: "SDDSLoader", code: 100, userInfo: [NSLocalizedDescriptionKey: "No columns found"]) }
