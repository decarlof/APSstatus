...
class DummyPVSource:
    ...
    def caget(self, pv_name):
        ...
        # Provide sensible, changing dummy values per PV
        if pv_name == "12bma:EnCalc":
            return 12.0 + 0.5 * math.sin(t / 2)

        if pv_name == "12bm_panda:POSITIONS:12:VAL":
            return 1000 + 50 * math.sin(t / 1.3)
        if pv_name == "12bm_panda:POSITIONS:13:VAL":
            return 2000 + 50 * math.sin(t / 1.7)
        if pv_name == "12bm_panda:POSITIONS:14:VAL":
            return 3000 + 50 * math.sin(t / 2.1)

        if pv_name == "12bm_xsp3:MaxDeadTime_RBV":
            return 10 + 5 * (0.5 + 0.5 * math.sin(t / 3))  # ~10–15%

        if pv_name == "S:SRcurrentAI.VAL":
            return 100.0 + 2.0 * math.sin(t / 4)

        # Shutters: return values matching your color logic
        if pv_name == "S12BM-PSS:FES:BeamBlockingM":
            return 0.0 if int(t) % 6 < 4 else 1.0  # green most of the time
        if pv_name == "S12BM-PSS:SBS:BeamPresentM":
            return 1.0 if int(t) % 6 < 4 else 0.0  # green most of the time
        if pv_name == "12bmb1:uniblitz:asyn.AOUT":
            return "A" if int(t) % 6 < 2 else "B"  # red sometimes

        return None


class PVMonitorApp:
    def __init__(self, root, pv_dict, caget_func):
        self.root = root
        self.root.title("EPICS PV Monitor")
        self.root.geometry("370x305")
        self.root.configure(bg="#2C2F33")

        self.pv_dict = pv_dict
        self.caget = caget_func  # injected data source (EPICS or dummy)
    ...
    def update_pv_values(self):
        ...
        for display_name, pv_name in self.pv_dict.items():
            value = self.caget(pv_name) or "N/A"
...
def main():
    ...
    # Use dummy source for GUI testing
    dummy = DummyPVSource()
    app = PVMonitorApp(root, pv_dict, caget_func=dummy.caget)
